...
while True:
    ...
    # 顔認識の実行
    face_rect = face_cascade.detectMultiScale(grayframe, scaleFactor=1.1, minNeighbors=3, minSize=(1, 1))
    ...
    if len(face_rect) > 0:
        ...
        for (x, y, w, h) in face_rect:
            cv2.rectangle(frame, (x, y), (x+w, y+h), color1)

            # 瞳の画像解析用にモノクロデータ生成
            egray = grayframe[y:y+h, x:x+w]
            # 瞳認識の実行
            eye_rect = eye_cascade.detectMultiScale(egray, scaleFactor=1.1, minNeighbors=3, minSize=(1, 1))
            print('eye_rect:')                                                    # 確認用
            print(eye_rect)                                                       # 確認用
            # 瞳描画用のフレームを設定
            eframe = frame[y:y+h, x:x+w]
            # cv2.rectangleで瞳に黄色枠を表示
            for (ex, ey, ew, eh) in eye_rect:
                cv2.rectangle(eframe, (ex, ey), (ex+ew, ey+eh), color2)
